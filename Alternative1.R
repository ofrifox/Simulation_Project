...
##----------------------------------------- 1.  all functions ------------------------------------------------
mm2envs <- mclapply(1:36, function(i) {
  set.seed(i+456)
  ...
  ## check if eush houer 
  check_rush<-function(time){
    if(time<=540 & time>=480){
      return(1)}
    else{
      return(0)
    }
  ...
  Train<- simmer("Train")%>%
    ...
    add_resource("exit_Hertzelia",capacity=3,queue_size=Inf)%>%   ##exit
    ...
    add_resource("exit_Rehovot",capacity=3,queue_size=Inf)%>%
    ...
    add_resource("platform_Lod",capacity=70,queue_size=0,preemptive = T)%>%
  ...
  TLV_coffee_rush <- trajectory("TLV_Coffee")%>%
    log_("Hi I want to buy Coffee")%>%
    seize("coffee_TLV",1)%>%
    timeout(function () get_attribute(Train, "coffee")*0.8)%>%
    release("coffee_TLV",1)
  
  TLV_toilet_rush <- trajectory("TLV_toilet")%>%
    log_("Hi I want to use the toilet")%>%
    seize("toilet_TLV",1)%>%
    timeout(function () get_attribute(Train, "toilet")*0.8)%>%
    release("toilet_TLV",1)
  
  
  TLV_rush <- trajectory("TLV")%>%
    log_("hello security, check me! ")%>%
    seize("security_TLV",1)%>%
    timeout(function () get_attribute(Train, "ticket")*0.8)%>%
    ...
    branch(option = function() {rdiscrete(1, c(0.2,0.5,0.3), c(0,1,2)) }, continue = c(T,T),TLV_coffee_rush, TLV_toilet_rush) %>%
    timeout(function () get_attribute(Train, "platform")*0.8)%>%
    branch( option= function()get_attribute(Train, "direction"),continue=c(T,T),TLV_platform_to_north ,TLV_platform_to_south )
  ...
  TLV <- trajectory("TLV")%>%
    branch( option= function() check_rush(now(Train)),continue=c(F),TLV_rush)%>%
    log_("hello security, check me! ")%>%
    seize("security_TLV",1)%>%
    timeout(function () get_attribute(Train, "ticket"))%>%
    release("security_TLV",1)%>%
...
Activity_time <- get_mon_arrivals(mm2envs, per_resource=TRUE)%>%
...
madad1_a1<- sqldf("select Hr.replication,  (Hr.Hertzelia_time + L.lod_time +T.TLV_time + He.Hedera_time+ R.Rehovot_time)/5 as mean_time
              from AVG_time_in_Hertzelia as Hr join AVG_time_in_lod as L on  Hr.replication=L.replication
              join AVG_time_in_TLV as T  on  Hr.replication=T.replication 
              join AVG_time_in_Hedera He  on  Hr.replication=He.replication
              join AVG_time_in_Rehovot as R  on  Hr.replication=R.replication
...
madad1_SD_and_mean_a1 <-  sqldf("select AVG(mean_time) as Mean, STDEV(mean_time) as Standard_Deviation
                                     from madad1_a1")


Madad_simulation_a1 <- unlist(madad1_SD_and_mean_a1[1,], use.names = FALSE)
madad_1_Mean_a1 <- Madad_simulation_a1[1]
madad_1_STDEV_a1 <- Madad_simulation_a1[2]
...
fullData<-get_mon_arrivals(mm2envs) 
arrivalData <- get_mon_arrivals(mm2envs, per_resource =TRUE)
...
madad2_a1 <- sqldf("select replication, avg(end_time-start_time) as AVG_Simulation_time  
            from fullData
            where finished=TRUE
           group by replication")
...
madad2_SD_and_mean_a1 <-  sqldf("select AVG(AVG_Simulation_time) as Mean, STDEV(AVG_Simulation_time) as Standard_Deviation
                                     from madad2_a1")


######Getting mean and stdev###########
madad_2_sd_mean_a1 <- unlist(madad2_SD_and_mean_a1[1,], use.names = FALSE)
madad2_Mean_a1 <- madad_2_sd_mean_a1[1]
madad2_STDEV_a1 <- madad_2_sd_mean_a1[2]
...
madad3_a1 <-sqldf("select a.replication, (cast(a.count_bus as real)/(select count  
                                                          from  Count_all as b 
                                                          join Count_bus as a  
...
replication_num <- data.frame(replication=1:36)  
madad3_a1_t <-sqldf("select a.replication,  b.bus_Proportion 
            from  replication_num as a left join madad3_a1 as b on a.replication=b.replication
           group by a.replication")

madad3_a1_t[is.na(madad3_a1_t)] <- 0  ## to t_test 



madad3_SD_and_mean_a1 <-  sqldf("select AVG(bus_Proportion) as Mean, STDEV(bus_proportion) as Standard_Deviation
                                     from madad3_a1")


######Getting mean and stdev###########
madad_3_Proportion_a1 <- unlist(madad3_SD_and_mean_a1[1,], use.names = FALSE)
madad_3_mean_a1 <- madad_3_Proportion_a1[1]
madad_3_stdev_a1 <- madad_3_Proportion_a1[2]
